...
std::vector<IncludeDirective> IncludeProcessing::getUnresolvedIncludeDirectives(const std::set<FilePath>& sourceFilePaths,
...
std::set<FilePath> IncludeProcessing::getHeaderSearchDirectories(const std::set<FilePath>& sourceFilePaths,
  ...
  for(size_t i = 0; i < parts.size(); i++) {
    ...
    while(!unprocessedFilePaths.empty()) {
      std::transform(unprocessedFilePaths.begin(),
...
std::vector<IncludeDirective> IncludeProcessing::doGetUnresolvedIncludeDirectives(
  ...
  while(!filePathsToProcess.empty()) {
    std::transform(filePathsToProcess.begin(),
