...
std::unique_ptr<clang::ASTConsumer> GeneratePCHAction::CreateASTConsumer(clang::CompilerInstance& CI, llvm::StringRef InFile) {
  ...
  Consumers.push_back(std::make_unique<clang::PCHGenerator>(CI.getPreprocessor(),
                                                            CI.getModuleCache(),
                                                            OutputFile,
                                                            Sysroot,
                                                            Buffer,
  ...
  Consumers.push_back(CI.getPCHContainerWriter().CreatePCHContainerGenerator(CI, InFile.str(), OutputFile, std::move(OS), Buffer));
