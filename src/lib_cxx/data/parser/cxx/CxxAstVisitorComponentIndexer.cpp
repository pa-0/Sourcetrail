  ...
  if(getAstVisitor()->shouldVisitReference(loc.getLocation())) {
    if(loc.getArgument().getKind() == clang::TemplateArgument::Template) {
      ...
      if(templateTemplateArgumentName.isDependent()) {
        ...
        m_client->recordReference(REFERENCE_TYPE_USAGE,
                                  symbolId,
        ...
        if(clang::CXXRecordDecl* declaringRecordDecl = clang::dyn_cast_or_null<clang::CXXRecordDecl>(d->getParent())) {
          if(clang::CXXRecordDecl* declaringRecordTemplateDecl = declaringRecordDecl->getTemplateInstantiationPattern()) {
            for(clang::Decl* templateMethodDecl : declaringRecordTemplateDecl->decls()) {
              if(clang::FunctionTemplateDecl* functionTemplateDecl = clang::dyn_cast_or_null<clang::FunctionTemplateDecl>(
                     templateMethodDecl)) {
                if(d->getDeclName().isIdentifier() && functionTemplateDecl->getDeclName().isIdentifier() &&
                  ...
                  m_client->recordReference(
                      REFERENCE_TEMPLATE_SPECIALIZATION, templateMethodId, symbolId, getParseLocation(d->getLocation()));
                  break;
                }
              }
            }
...
void CxxAstVisitorComponentIndexer::visitCXXMethodDecl(clang::CXXMethodDecl* d) {
  ...
  if(getAstVisitor()->shouldVisitDecl(d)) {
    ...
    for(clang::CXXMethodDecl::method_iterator it = d->begin_overridden_methods();    // TODO: iterate in traversal and use
...
void CxxAstVisitorComponentIndexer::visitNamespaceAliasDecl(clang::NamespaceAliasDecl* d) {
  if(getAstVisitor()->shouldVisitDecl(d)) {
    ...
    m_client->recordReference(
        REFERENCE_USAGE, getOrCreateSymbolId(d->getAliasedNamespace()), symbolId, getParseLocation(d->getTargetNameLoc()));
...
void CxxAstVisitorComponentIndexer::visitDeclRefExpr(clang::DeclRefExpr* s) {
  ...
  if(getAstVisitor()->shouldVisitReference(s->getLocation())) {
    if((clang::isa<clang::ParmVarDecl>(decl)) ||
      ...
      m_client->recordReference(refKind,
                                symbolId,
                                getOrCreateSymbolId(getAstVisitor()->getComponent<CxxAstVisitorComponentContext>()->getContext()),
...
void CxxAstVisitorComponentIndexer::visitConstructorInitializer(clang::CXXCtorInitializer* init) {
  if(getAstVisitor()->shouldVisitReference(init->getMemberLocation())) {
    ...
    if(clang::FieldDecl* memberDecl = init->getMember()) {
      m_client->recordReference(REFERENCE_USAGE,
...
void CxxAstVisitorComponentIndexer::recordTemplateMemberSpecialization(const clang::MemberSpecializationInfo* memberSpecializationInfo,
                                                                       Id contextId,
...
Id CxxAstVisitorComponentIndexer::getOrCreateSymbolId(const clang::NamedDecl* decl) {
  ...
  if(decl) {
    std::unique_ptr<CxxDeclName> declName = CxxDeclNameResolver(getAstVisitor()->getCanonicalFilePathCache()).getName(decl);
    if(declName) {
      ...
      if(dynamic_cast<CxxFunctionDeclName*>(declName.get()) && symbolName.size() == 1 && symbolName.back().getName() == L"main") {
        ...
        symbolName.pop();
        symbolName.push(NameElement(L".:main:." + getAstVisitor()->getCanonicalFilePathCache()->getDeclarationFilePath(decl).wstr(),
                                    sig.getPrefix(),
                                    sig.getPostfix()));
      }
    }
...
Id CxxAstVisitorComponentIndexer::getOrCreateSymbolId(const clang::Type* type) {
  ...
  NameHierarchy symbolName(L"global", NAME_DELIMITER_UNKNOWN);
  if(type) {
    std::unique_ptr<CxxTypeName> typeName = CxxTypeNameResolver(getAstVisitor()->getCanonicalFilePathCache()).getName(type);
    if(typeName) {
