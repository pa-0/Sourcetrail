...
CxxTemplateParameterStringResolver::CxxTemplateParameterStringResolver(const CxxNameResolver* other) : CxxNameResolver(other) {}
...
std::wstring CxxTemplateParameterStringResolver::getTemplateParameterTypeString(const clang::NonTypeTemplateParmDecl* parameter) {
  std::wstring typeString = CxxTypeName::makeUnsolvedIfNull(CxxTypeNameResolver(this).getName(parameter->getType()))->toString();
