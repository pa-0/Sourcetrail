...
CxxDeclNameResolver::CxxDeclNameResolver(const CxxNameResolver* other) : CxxNameResolver(other), m_currentDecl(nullptr) {}
...
std::unique_ptr<CxxDeclName> CxxDeclNameResolver::getName(const clang::NamedDecl* declaration) {
  ...
  if(declName) {
    ...
    } else if(!clang::isa<clang::TemplateTypeParmDecl>(declaration) && !clang::isa<clang::NonTypeTemplateParmDecl>(declaration) &&
  ...
  if(declaration) {
    ...
    if(const clang::RecordDecl* recordDecl = clang::dyn_cast_or_null<clang::RecordDecl>(declaration)) {
      ...
      if((clang::dyn_cast_or_null<clang::CXXMethodDecl>(functionDecl)) &&
        ...
        for(unsigned i = 0; i < templateArgumentList->size(); i++) {
          ...
          if(templateArgument.isDependent()) {
            if(clang::FunctionTemplateDecl* tempFunctionDeclaration = functionDecl->getPrimaryTemplate()) {
      ...
      if(!clang::isa<clang::CXXMethodDecl>(declaration) && isStatic) {
        return std::make_unique<CxxStaticFunctionDeclName>(std::move(functionName),
      ...
      return std::make_unique<CxxFunctionDeclName>(std::move(functionName),
                                                   std::move(templateArguments),
                                                   std::move(returnTypeName),
                                                   std::move(parameterTypeNames),
                                                   isConst,
      ...
      if(varDecl->getParentFunctionOrMethod() == nullptr) {
        ...
        if(utility::getSymbolKind(varDecl) == SYMBOL_GLOBAL_VARIABLE && varDecl->getStorageClass() == clang::SC_Static) {
...
std::wstring CxxDeclNameResolver::getNameForAnonymousSymbol(const std::wstring& symbolKindName, const clang::Decl* declaration) {
  ...
  if(presumedBegin.isValid()) {
    return L"anonymous " + symbolKindName + L" (" + getCanonicalFilePathCache()->getDeclarationFileName(declaration) + L'<' +
        std::to_wstring(presumedBegin.getLine()) + L':' + std::to_wstring(presumedBegin.getColumn()) + L">)";
  }
