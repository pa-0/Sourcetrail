...
void CxxAstVisitorComponentBraceRecorder::visitTagDecl(clang::TagDecl* d) {
  if(getAstVisitor()->shouldVisitDecl(d)) {
    if(d->isThisDeclarationADefinition() &&
      ...
      recordBraces(getFilePath(d->getBraceRange().getBegin()),
...
void CxxAstVisitorComponentBraceRecorder::visitCompoundStmt(clang::CompoundStmt* s) {
  if(getAstVisitor()->shouldVisitStmt(s)) {
    ...
    if(!contextDecl || !utility::isImplicit(contextDecl)) {
      recordBraces(getFilePath(s->getLBracLoc()), getParseLocation(s->getLBracLoc()), getParseLocation(s->getRBracLoc()));
    }
  }
}
...
void CxxAstVisitorComponentBraceRecorder::visitInitListExpr(clang::InitListExpr* s) {
  if(getAstVisitor()->shouldVisitStmt(s)) {
    if(s->isSyntacticForm()) {
      const clang::NamedDecl* contextDecl = getAstVisitor()->getComponent<CxxAstVisitorComponentContext>()->getTopmostContextDecl();
      if(!contextDecl || !utility::isImplicit(contextDecl)) {
        recordBraces(getFilePath(s->getLBraceLoc()), getParseLocation(s->getLBraceLoc()), getParseLocation(s->getRBraceLoc()));
      }
    }
  }
}
...
void CxxAstVisitorComponentBraceRecorder::visitMSAsmStmt(clang::MSAsmStmt* s) {
  if(getAstVisitor()->shouldVisitStmt(s)) {
    if(s->hasBraces()) {
      const clang::NamedDecl* contextDecl = getAstVisitor()->getComponent<CxxAstVisitorComponentContext>()->getTopmostContextDecl();
      if(!contextDecl || !utility::isImplicit(contextDecl)) {
        recordBraces(getFilePath(s->getLBraceLoc()),
...
void CxxAstVisitorComponentBraceRecorder::recordBraces(const FilePath& filePath,
                                                       const ParseLocation& lbraceLoc,
                                                       const ParseLocation& rbraceLoc) {
  if(lbraceLoc.startColumnNumber != rbraceLoc.startColumnNumber || lbraceLoc.endColumnNumber != rbraceLoc.endColumnNumber ||
    ...
    if(lbraceLoc.startColumnNumber == lbraceLoc.endColumnNumber && lbraceLoc.startLineNumber == lbraceLoc.endLineNumber) {
      m_client->recordLocalSymbol(name, lbraceLoc);
    }
    if(rbraceLoc.startColumnNumber == rbraceLoc.endColumnNumber && rbraceLoc.startLineNumber == rbraceLoc.endLineNumber) {
      m_client->recordLocalSymbol(name, rbraceLoc);
    }
  }
}

clang::SourceLocation CxxAstVisitorComponentBraceRecorder::getFirstLBraceLocation(clang::SourceLocation searchStartLoc,
...
clang::SourceLocation CxxAstVisitorComponentBraceRecorder::getLastRBraceLocation(clang::SourceLocation searchStartLoc,
                                                                                 clang::SourceLocation searchEndLoc) const {
  const clang::SourceManager& sm = m_astContext->getSourceManager();
  const clang::LangOptions& opts = m_astContext->getLangOpts();
