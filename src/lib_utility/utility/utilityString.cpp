...
template <typename StringType>
StringType doReplaceBetween(const StringType& str,
...
std::wstring decodeFromUtf8(const std::string& text) {
  return boost::locale::conv::utf_to_utf<wchar_t>(text.c_str(), text.c_str() + text.size());
}
...
std::wstring convertWhiteSpacesToSingleSpaces(const std::wstring& str) {
  ...
  for(size_t i = 1; i <= parts.size(); i++) {
    if(parts[i - 1].empty()) {
      parts.erase(parts.begin() + static_cast<int64_t>(i) - 1);
      i--;
    }
  }
...
bool caseInsensitiveLess(const std::wstring& s1, const std::wstring& s2) {
  ...
  for(size_t i = 0; i < lesser_size; ++i) {
    ...
    if(s1_wchr != s2_wchr) {
      s1_wchr = static_cast<wchar_t>(towlower(static_cast<wint_t>(s1_wchr)));
      s2_wchr = static_cast<wchar_t>(towlower(static_cast<wint_t>(s2_wchr)));
      if(s1_wchr != s2_wchr) {
