...
GraphViewStyle::NodeStyle::NodeStyle()
    : cornerRadius(0), borderWidth(0), borderDashed(false), fontSize(0), fontBold(false), iconSize(0), hasHatching(false) {}

GraphViewStyle::EdgeStyle::EdgeStyle()
    : width(0), zValue(0), arrowLength(0), arrowWidth(0), arrowClosed(false), cornerRadius(0), verticalOffset(0), dashed(false) {}
...
void GraphViewStyle::loadStyleSettings() {
  ...
  s_gridCellPadding = static_cast<int>(getCharHeight(NodeType::STYLE_BIG_NODE) - Offset);
...
GraphViewStyle::NodeStyle GraphViewStyle::getStyleOfBundleNode(bool isFocused) {
  return getStyleForNodeType(NodeType::STYLE_BIG_NODE,
                             "bundle",
...
int GraphViewStyle::toGridOffset(int x) {
  if(x > 0) {
    return static_cast<int>(std::ceil(x / double(s_gridCellPadding + s_gridCellSize)) * (s_gridCellPadding + s_gridCellSize));
  } else {
    return static_cast<int>(std::floor(x / double(s_gridCellPadding + s_gridCellSize)) * (s_gridCellPadding + s_gridCellSize));
  }
}
...
float GraphViewStyle::getCharWidth(NodeType::StyleType type) {
  auto iterator = s_charWidths.find(type);
  if(iterator != s_charWidths.end()) {
...
float GraphViewStyle::getCharHeight(NodeType::StyleType type) {
  auto iterator = s_charHeights.find(type);
  if(iterator != s_charHeights.end()) {
    return iterator->second;
  }
