...
bool SourceGroupSettingsWithExcludeFilters::equals(const SourceGroupSettingsBase* other) const {
  const SourceGroupSettingsWithExcludeFilters* otherPtr = dynamic_cast<const SourceGroupSettingsWithExcludeFilters*>(other);
    ...
    if(!filterString.empty()) {
      ...
      if(wildcardPos != filterString.npos) {
        ...
        if(std::regex_search(filterString, match, std::wregex(L"[\\\\/]")) && !match.empty() &&
          ...
          utility::append(
              result, utility::convert<FilePath, FilePathFilter>(utility::toVector(symLinkPaths), [match](const FilePath& filePath) {
        ...
        utility::append(
            result, utility::convert<FilePath, FilePathFilter>(utility::toVector(symLinkPaths), [isFile](const FilePath& filePath) {
