...
fs::path ApplicationSettings::getPluginDir() const {
  const auto appDir = fs::path {
      QCoreApplication::instance()->applicationDirPath().toStdString()};
  return fs::canonical(appDir / "plugins");
}