...
SharedMemory::ScopedAccess::ScopedAccess(SharedMemory* memory)
  ...
  try {
    ...
    m_memory = boost::interprocess::managed_shared_memory(
        boost::interprocess::open_or_create, memory->getMemoryName().c_str(), memory->getInitialMemorySize(), 0, permissions);
  }
}
...
std::string SharedMemory::ScopedAccess::logString() const {
  ...
  log += " used pct: " + std::to_string(static_cast<int>(100.0F - float(getFreeMemorySize()) / float(getMemorySize()) * 100.0F));
  return log;
}
