...
std::vector<NodeType> const NodeType::overviewBundleNodeTypesOrdered = {NodeType(NODE_FILE),
                                                                        NodeType(NODE_MACRO),
...
bool NodeType::isUsable() const {
  const NodeKindMask mask = NODE_SYMBOL | NODE_BUILTIN_TYPE | NODE_STRUCT | NODE_CLASS | NODE_ENUM | NODE_UNION | NODE_INTERFACE |
      NODE_ANNOTATION | NODE_TYPEDEF;
  return ((m_kind & mask) > 0);
}

bool NodeType::isPotentialMember() const {
  const NodeKindMask mask = NODE_METHOD | NODE_FIELD | NODE_CLASS | NODE_INTERFACE | NODE_ANNOTATION | NODE_STRUCT | NODE_UNION |
      NODE_TYPEDEF | NODE_ENUM;
...
bool NodeType::hasSearchFilter() const {
  const NodeKindMask mask = NODE_BUILTIN_TYPE | NODE_MODULE | NODE_NAMESPACE | NODE_PACKAGE | NODE_STRUCT | NODE_CLASS |
      NODE_INTERFACE | NODE_ANNOTATION | NODE_GLOBAL_VARIABLE | NODE_FIELD | NODE_FUNCTION | NODE_METHOD | NODE_ENUM |
      NODE_ENUM_CONSTANT | NODE_TYPEDEF | NODE_FILE | NODE_MACRO | NODE_UNION;
  return ((m_kind & mask) > 0);
}
