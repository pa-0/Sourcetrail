...
SourceLocation* SourceLocationCollection::addSourceLocation(LocationType type,
  ...
  return file->addSourceLocation(type,
                                 locationId,
                                 std::move(tokenIds),
                                 startLineNumber,
                                 startColumnNumber,
...
void SourceLocationCollection::addSourceLocationCopies(const SourceLocationCollection* other) {
  other->forEachSourceLocationFile([this](const std::shared_ptr<SourceLocationFile>& otherFile) {
...
void SourceLocationCollection::forEachSourceLocationFile(
    const std::function<void(std::shared_ptr<SourceLocationFile>)>& func) const {
  for(const auto& pFile : m_files) {
    func(pFile.second);
  }
}

void SourceLocationCollection::forEachSourceLocation(
...
std::wostream& operator<<(std::wostream& ostream, const SourceLocationCollection& base) {
  ...
  base.forEachSourceLocationFile(
      [&ostream](const std::shared_ptr<SourceLocationFile>& pSourceLocationFile) {
        ostream << *(pSourceLocationFile);
      });
  return ostream;
}