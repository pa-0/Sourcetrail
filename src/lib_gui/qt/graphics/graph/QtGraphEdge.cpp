    ...
    if((type == Edge::EDGE_INHERITANCE || type == Edge::EDGE_TEMPLATE_SPECIALIZATION) &&
      ...
      if(style.zValue < 0) {
        style.zValue += 5;
              ...
              (type == Edge::EDGE_TEMPLATE_SPECIALIZATION && owner == ownerNonGroupParent && target == targetNonGroupParent)) {
...
void QtGraphEdge::onClick() {
  if(isExpandable()) {
    ...
    MessageActivateTrailEdge(
        {getData()->getId()}, getData()->getType(), getData()->getFrom()->getNameHierarchy(), getData()->getTo()->getNameHierarchy())
        .dispatch();
  } else {
    TokenComponentInheritanceChain* componentInheritance = getData()->getComponent<TokenComponentInheritanceChain>();
    ...
    if(getData()->getType() == Edge::EDGE_BUNDLED_EDGES) {
      msg.bundledEdgesIds = utility::toVector<Id>(getData()->getComponent<TokenComponentBundledEdges>()->getBundledEdgesIds());
