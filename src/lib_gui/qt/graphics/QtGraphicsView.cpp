...
void QtGraphicsView::resizeEvent(QResizeEvent* pEvent) {
  m_focusIndicator->setGeometry(QRect(0, 0, pEvent->size().width(), 3));
  m_zoomState->setGeometry(QRect(31, pEvent->size().height() - 27, 65, 19));
  m_zoomInButton->setGeometry(QRect(8, pEvent->size().height() - 50, 19, 19));
  m_zoomOutButton->setGeometry(QRect(8, pEvent->size().height() - 27, 18, 19));
  m_pLegendButton->setGeometry(QRect(pEvent->size().width() - 24, pEvent->size().height() - 24, 18, 18));

  m_zoomInButton->setIconSize(QSize(15, 15));
  m_zoomOutButton->setIconSize(QSize(15, 15));
  m_pLegendButton->setIconSize(QSize(10, 10));
...
void QtGraphicsView::legendClicked() const {
...
void QtGraphicsView::handleMessage(MessageSaveAsImage* pMessage) {
  if((pMessage->getSchedulerId() == getSchedulerId()) && !m_imageCached.isNull()) {
    m_imageCached.save(pMessage->path);
  }
}
