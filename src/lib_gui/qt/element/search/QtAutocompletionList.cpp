...
void QtAutocompletionDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
  ...
  if(type.size() && type != QLatin1String("command") && type != QLatin1String("filter")) {
    ...
    fillColor = QColor(
        scheme->getSearchTypeColor(utility::encodeToUtf8(SearchMatch::getSearchTypeName(SearchMatch::SEARCH_COMMAND)), "fill").c_str());
    textColor = QColor(
        scheme->getSearchTypeColor(utility::encodeToUtf8(SearchMatch::getSearchTypeName(SearchMatch::SEARCH_COMMAND)), "text").c_str());
  }
  ...
  if(!indices.empty()) {
    ...
    QRect rect(
        option.rect.left(), option.rect.top() + top1, static_cast<int>(m_charWidth1 - 1), static_cast<int>(m_charHeight1 - 2));
    painter->fillRect(rect, fillColor);
  }
  ...
  painter->drawText(option.rect.adjusted(static_cast<int>(m_charWidth1 + 2), top1 - 3, 0, 0), Qt::AlignLeft, highlightText);
  painter->restore();
  ...
  if(subtext.size()) {
    ...
    // draw subtext normal
    painter->drawText(option.rect.adjusted(static_cast<int>((3 * m_charWidth2) + 2), top2, 0, 0), Qt::AlignLeft, subtext);
    ...
    painter->drawText(option.rect.adjusted(static_cast<int>((3 * m_charWidth2) + 2), top2, 0, 0), Qt::AlignLeft, highlightSubtext);
    painter->restore();
  }
