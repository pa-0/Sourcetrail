...
void QtCodeFileList::scrollTo(const FilePath& filePath,
  ...
  if(focusTarget) {
    m_navigator->setFocusedLocationId(
        snippet->getArea(), lineNumber, snippet->getArea()->getColumnNumberForLocationId(locationId), locationId, {}, false, false);
  }
}
  ...
  if(m_mirroredTitleBar != mirroredTitleBar) {
    ...
    if(m_mirroredTitleBar && file) {
      ...
      m_firstSnippetTitleBar->setGeometry(
          file->pos().x() + mirroredTitleBar->pos().x(), 0, mirroredTitleBar->width(), mirroredTitleBar->height());
