...
QtCodeFileTitleButton::QtCodeFileTitleButton(QWidget* parent)
    : QtSelfRefreshIconButton(QLatin1String(""), FilePath(), "code/file/title", parent), m_isComplete(true), m_isIndexed(true) {
...
void QtCodeFileTitleButton::updateHatching() {
  if(!m_isIndexed) {
    FilePath hatchingFilePath = ResourcePaths::getGuiDirectoryPath().concatenate(
        L"code_view/images/pattern_" + utility::decodeFromUtf8(ColorScheme::getInstance()->getColor("code/file/title/hatching")) +
        L".png");

    setStyleSheet(QString::fromStdWString(L"#title_button { background-image: url(" + hatchingFilePath.wstr() + L"); }"));
  } else {
