...
void QtCodeNavigator::updateReferenceCount(size_t referenceCount,
  ...
  m_refLabel->setMinimumWidth(
      m_refLabel->fontMetrics().boundingRect(QString(QString::number(referenceCount).size() * 2, 'a') + "/ references").width() +
      30);
...
void QtCodeNavigator::activateScreenMatch(size_t matchIndex) {
  ...
  scrollTo(CodeScrollParams::toReference(p.first->getFilePath(), m_activeScreenMatchId, 0, CodeScrollParams::Target::CENTER),
           true,
           true);
}
...
void QtCodeNavigator::scrollToFocus() {
  ...
  if(focus.file) {
    ...
    scrollTo(CodeScrollParams::toReference(focus.area->getFilePath(), focus.locationId, 0, CodeScrollParams::Target::VISIBLE),
             true,
             false);
  }
}
