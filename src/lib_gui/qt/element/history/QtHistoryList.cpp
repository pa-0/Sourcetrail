...
QtHistoryItem::QtHistoryItem(const SearchMatch& match, size_t index_, bool isCurrent) : index(index_), m_match(match) {
  ...
  const std::wstring name = utility::elide(match.getFullName(), utility::ElideMode::RIGHT, 100);
  ...
  if(isCurrent) {
    QtDeviceScaledPixmap pixmap(
        QString::fromStdWString(ResourcePaths::getGuiDirectoryPath().concatenate(L"history_list/images/arrow.png").wstr()));
...
QtHistoryList::QtHistoryList(const std::vector<SearchMatch>& history, size_t currentIndex) : m_currentIndex(currentIndex) {
  ...
  setStyleSheet(utility::getStyleSheet(ResourcePaths::getGuiDirectoryPath().concatenate(L"history_list/history_list.css")).c_str());
